...
class JournalpostInstantMappingService : InstanceMapper<Journalpost> {
    ...
    override fun map(
        sourceApplicationId: Long, incomingInstance: Journalpost, persistFile: PersistFile
    ): InstanceObject {
        return with(incomingInstance) {

            val dokumenterInstanceObjects = mapAttachmentDocumentsToInstanceObjects(
            ...
            val valuePerKey: Map<String, String> = buildMap {
            ...
            val objectCollectionPerKey = mutableMapOf<String, Collection<InstanceObject>>(
    ...
    private fun toFile(
        sourceApplicationId: Long, sourceApplicationInstanceId: String, dokument: Dokument, type: MediaType
    ): File {
        return File(
    ...
    private fun mapAttachmentDocumentToInstanceObject(
        persistFile: PersistFile, sourceApplicationId: Long, sourceApplicationInstanceId: String, dokument: Dokument
    ): InstanceObject {
    ...
    private fun mapAttachmentDocumentAndFileIdToInstanceObject(
        dokument: Dokument, mediaType: MediaType, fileId: UUID
    ): InstanceObject {
        return InstanceObject(
            valuePerKey = buildMap {
